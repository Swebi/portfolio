...
const DEFAULT_SCREEN_HEIGHT = 700
...
export interface SafariProps extends HTMLAttributes<HTMLDivElement> {
  ...
  videoSrc?: string
  mode?: SafariMode
  contentAspectRatio?: number // width/height, e.g. 16/9
}
...
export function Safari({
  ...
  const SCREEN_HEIGHT = contentAspectRatio
    ? Math.round(SCREEN_WIDTH / contentAspectRatio)
    : DEFAULT_SCREEN_HEIGHT
  const SAFARI_HEIGHT = SCREEN_Y + SCREEN_HEIGHT + 1
  const CURVE_Y = SAFARI_HEIGHT - 12
  ...
  const punchPath = `M1 52H1201V${CURVE_Y}C1201 ${CURVE_Y + 6.075} 1196.08 ${SAFARI_HEIGHT - 1} 1190 ${SAFARI_HEIGHT - 1}H12C5.92486 ${SAFARI_HEIGHT - 1} 1 ${CURVE_Y + 6.075} 1 ${CURVE_Y}V52Z`
  const fillPath = `M0 52H1202V${CURVE_Y}C1202 ${CURVE_Y + 6.627} 1196.63 ${SAFARI_HEIGHT} 1190 ${SAFARI_HEIGHT}H12C5.37258 ${SAFARI_HEIGHT} 0 ${CURVE_Y + 6.627} 0 ${CURVE_Y}V52Z`

  return (
    <div
      className={`relative inline-block w-full align-middle leading-none overflow-hidden rounded-[11px] ${className ?? ""}`}
      style={{
    ...
    >
      ...
      >
        ...
        >
          <path d={fillPath} className="fill-[#E5E5E5] dark:fill-[#404040]" />
          <path
            fillRule="evenodd"
            clipRule="evenodd"
